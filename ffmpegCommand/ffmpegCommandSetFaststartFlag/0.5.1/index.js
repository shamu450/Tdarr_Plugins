...
var plugin = function (args) {
    ...
    const extension = fileUtils_1.getContainer(args.inputFileObj._id).toLowerCase(); // remove fileUtils_1 when moving to ts
    let hasFlag = false;
    ...
    if (extensionArray.includes(extension)) { // verify container type before continuing
        ...
        args.jobLog(`\u2716 Found container: ${extension}`); // jobLog message setting flag on file
        args.jobLog(`\u2714 Setting faststart flag, but flag will be ignored unless remuxed to MOV family container`); // jobLog message setting flag on file
        args.variables.ffmpegCommand.overallOuputArguments.push('-movflags', 'faststart'); // set new codec tag with ffmpeg output variables
        hasFlag = true; // set hasFlag to true
    }
